 ...
 * @LastEditTime: 2024-06-25 14:23:56
 * @Description: 用户管理
 */
...
function UserManage() {
  ...
  const roleList = useRef<Array<{ roleName: string; id: number }>>([]);
              ...
              (validate as Function)().then(async (value: Record<string, any>) => {
                const params =
                  type === OperateType.EDIT
                    ? {
                        ...pick(value, ["role", "status", "remark", "password"]),
                        ...pick(data, ["id", "status"]),
                      }
                    : value;
                ...
                const [error] = await to(
                  type === OperateType.EDIT ? updateUser(params) : createUser(role, params)
                );
                if (!error) {
                  getData(paginationConfig);
    ...
    <div className="user-box">
      <div className="flex justify-end mb-3">
        ...
        <CustomShowContainer id={2}>
          <Button type="primary" onClick={() => handleOperate(OperateType.ADD)}>
            添加用户
          </Button>
        </CustomShowContainer>
      </div>
