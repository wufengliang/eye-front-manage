 ...
 * @LastEditTime: 2024-06-02 21:49:50
 * @Description: 界面布局
 */
...
import { setRoleList as setRoleListMapData } from '@/store/role';
...
function LayoutBox() {
  ...
  const { userInfo, menuList } = useSelector((state: Record<string, any>) => state.user);
  ...
  useEffect(() => {
    ...
    setMenus(menuList.map((i: any) => ({ key: i.path.slice(1), label: i.title, element: routerMap[i.title]?.element, icon: i.icon })))
  }, [menuList, userInfo]);
