...
function EditQuestion(props: IQuestionItemType) {
  ...
  const { question, choiceMatrices } = props.value;
  ...
  useEffect(() => {
    if (selectTitleType === 4) {
      form.setFieldValue(["question", "questionGroupId"], undefined);
    }
  }, [selectTitleType]);
  ...
  const saveQuesition = async () => {
    ...
    if (result) {
      ...
      if (question.type === 0) {
        const newChoicePrepares = (choicePrepares || []).map(
          (item: Record<string, any>, i: number) => ({
            prepareName: item.value,
            prepareOrder: i + 1,
            flag: questionAnswer === i,
          })
        );
      ...
      Object.assign(
        ...
        {
          questionFiles: (questionFiles ?? []).map((item: Record<string, any>, index: number) => ({
            filePath: item.url,
            fileOrder: index + 1,
          })),
        }
      ...
      const [, value] = await to(
        question?.id ? updateQuestion(postParams) : addQuestion(postParams)
      );
      if (value) {
  ...
  const renderMatrixBox = () => {
    ...
    const columnChildren = newChoiceMatrices.filter(
      (item: Record<string, any>) => !!item.matrixType
    );
                ...
                {Array.from({ length: columnChildren.length + 1 }).map((k, i) => {
                  return (
                    <>
                      <div
                        className="mx-1"
                        style={{ width: `${(24 * 100) / (columnChildren.length + 2)}%` }}
                      >
                        {index === 0 && i === 0 ? (
                        ...
                        onClick={() => {
                          setChoiceMatrices([
                            ...newChoiceMatrices,
                            {
                              matrixType: true,
                              matrixName: `默认值`,
                              matrixOrder: columnChildren.length,
                            },
                          ]);
                        }}
                      />
                    ...
                    onClick={() => {
                      setChoiceMatrices([
                        ...newChoiceMatrices,
                        {
                          matrixType: false,
                          matrixName: `默认值`,
                          matrixOrder: rowChildren.length,
                        },
                      ]);
                    }}
                  />
  ...
  return (
    <div className="bg-[#fafafa] p-4">
      <Form
        ...
        wrapperCol={{ span: 16 }}
        form={form}
        initialValues={{ ...props.value, testType: 0 }}
      >
        <Form.Item
          label="题目"
          className="py-1"
        ...
        <Form.Item label="题目类型" className="py-1" name={["question", "type"]}>
          <Select placeholder="请选择题目类型..." disabled>
            {QUESTTION_TYPE_LIST.map((item, index) => (
        ...
        ) : null} */}
        {
          <>
            <Form.Item
              label="题目归属"
              name={["question", "questionGroupId"]}
              rules={selectTitleType !== 4 ? [{ required: true, message: "请选择题目归属" }] : []}
            >
              <Select placeholder="请选择题目归属...">
                {questionGroups.map((i) => (
                  <>
          ...
          <>
            <Form.Item
              ...
              name={
                selectTitleType === 1
                  ? "questionFiles"
                  : ["question", [2, 3].includes(selectTitleType) ? "audioPath" : ""]
              }
            >
              <CustomUpload
        ...
        {/* 选项是否随机 */}
        {[4, 5].includes(selectQuestionType) ? null : (
          <Form.Item
            label="列选项是否随机"
            className="py-1"
            name={["question", "choiceRandom"]}
            rules={[{ required: true, message: "请选择列选项是否随机" }]}
          >
            <Radio.Group>
                            ...
                            <Space className="flex items-baseline">
                              ...
                              >
                                <Select placeholder="请选择..." style={{ width: `100px` }}>
                                  ...
                                  <Select.Option value={2} disabled={selectQuestionType === 0}>
                                    图片
                                  </Select.Option>
                                </Select>
                              </Form.Item>
                              <Form.Item
                                shouldUpdate={(prevValues, currentValues) =>
                                  prevValues.choicePrepares[name]?.type !==
                                  currentValues.choicePrepares[name]?.type
                                }
                              >
                                {({ getFieldValue }) => {
                              ...
                              {Array.isArray(selectchoicePrepares) &&
                              selectchoicePrepares.length > 0 ? (
                          ...
                          <Row key={key}>
                            <Col span={16} offset={6}>
                              <Space className="flex items-baseline">
                                ...
                                <Form.Item
                                  shouldUpdate={(prevValues, currentValues) =>
                                    prevValues.choiceOptions[name]?.type !==
                                    currentValues.choiceOptions[name]?.type
                                  }
                                >
                                  {({ getFieldValue }) => {
                ...
                {(selectchoicePrepares || []).map((item: Record<string, any>, index: number) => (
                  <>
                    <Select.Option value={index}>
                      {item?.prepareName ?? item?.value ?? `选项${index + 1}`}
                    </Select.Option>
                  </>
                ))}
              </Select>
        ...
        <Form.Item wrapperCol={{ span: 16, offset: 4 }} className="py-1">
          <div className="flex justify-end">
            <Button className="mx-4" onClick={() => onChange?.(OperateType.CANCEL, index, props)}>
              取消编辑
            </Button>
            <Button type="primary" onClick={() => saveQuesition()}>
              确定
            </Button>
          </div>
        </Form.Item>
