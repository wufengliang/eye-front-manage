 ...
 * @LastEditTime: 2024-07-09 10:53:41
 * @Description: 项目编辑
 */
...
function ProjectEdit() {
  ...
  const getData = async () => {
    ...
    if (!error) {
      result &&
        setList(
          (result || []).map((item: Record<string, any>) => ({ isEditMode: false, value: item }))
        );
    }
  };
    ...
    switch (type) {
        ...
        return setList(
          [...newList].concat({
            ...omit(
              {
                ...data,
                value: { ...data?.value, question: { ...omit(data?.value?.question, ["id"]) } },
              },
              ["index"]
            ),
            isEditMode: true,
      ...
      case OperateType.EDIT:
        ...
        newList.splice(index, 1, {
          ...
          value: {
            ...data?.value,
            questionFiles: (data?.value?.questionFiles || []).map((item: Record<string, any>) => {
        ...
        return setList(
          newList.map((item) => ({
            ...
            value: {
              ...item.value,
              choiceOptions: (item?.value?.choiceOptions || []).map((v: any) => ({
                ...v,
                type: !!v.optionImage ? 2 : 1,
                value: !!v.optionImage ? v.optionImage : v.optionName,
              })),
              choicePrepares: (item?.value?.choicePrepares || []).map((v: any) => ({
                ...v,
                value: v.prepareName,
              })),
  ...
  const saveSurvey = async () => {
    ...
    if (value) {
      ...
      if (!error) {
        checked.current
          ? message.warning(`警告：如果已收集数据，更改问卷问题会导致数据错乱！`)
      ...
      <div className="right-box px-3 sticky top-0 h-full">
        ...
        <div className="mt-5">
          <ProjectTemplate
            ...
            ref={projectRef}
            {...Object.assign({}, state, {
              startTime: dayjs(state.startTime),
              endTime: dayjs(state.endTime),
            })}
          />
        </div>
      </div>
          ...
          <Tabs
            ...
            items={[
                        ...
                        renderItem={(item: any) => {
                          return item?.isEditMode ? (
                            <EditQuestion
                              ...
                              ) => handleOperate(type, index, currentValue)}
                            ...
                            <QuestionItem
                              ...
                              {...Object.assign({}, item, {
                                value: {
                                  ...item.value,
                                  choiceOptions: (item.value?.choiceOptions || []).map(
                                    (v: Record<string, any>) => ({
                                      ...v,
                                      type: v.optionImage ? 2 : 1,
                                      value: v?.optionImage || v.optionName,
                                    })
                                  ),
                                },
                              })}
              ...
              },
              {
                label: "结束页",
                key: "2",
                children: <p className="text-center">{state?.endTips}</p>,
              },
            ]}
          />
        </div>
