 ...
 * @LastEditTime: 2024-07-04 14:57:55
 * @Description: 测试视频
 */
...
const getData = (
  ...
  const newForm = { ...form };
  newForm.surveyId ? (newForm.surveyId = Number(newForm.surveyId)) : delete newForm.surveyId;
  !newForm.userId && delete newForm.userId;
  return getTestVideoList({
    page: params.current,
...
function TestVideo() {
  ...
  const { tableProps, search } = useAntdTable(getData, {
    defaultParams: [{ current: 1, pageSize: 10, all: 1 }, { search: "" }],
      ...
      render: (_, record) => (
        <>
          <Button
            ...
            onClick={() => handleOperate(OperateType.PLAY, record)}
          >
            点击播放
          </Button>
          <Button
            type="primary"
            className="margin-right-10 margin-bottom-5"
            onClick={() => handleOperate(OperateType.DETAIL, record, TTestVideoType.HOT)}
          >
            查看热力图
          </Button>
          <Button
            type="primary"
            className="margin-right-10 margin-bottom-5"
            onClick={() => handleOperate(OperateType.DETAIL, record, TTestVideoType.TRAJECTORY)}
          >
            查看轨迹图
          </Button>
          <Button
            type="primary"
            className="margin-right-10 margin-bottom-5"
            onClick={() => handleOperate(OperateType.DOWNLOAD, record, TTestVideoType.HOT)}
          >
            下载热力图
          </Button>
          <Button
            type="primary"
            className="margin-right-10 margin-bottom-5"
            onClick={() => handleOperate(OperateType.DOWNLOAD, record, TTestVideoType.TRAJECTORY)}
          >
            下载轨迹图
          </Button>
          <Button
            type="primary"
            className="margin-right-10 margin-bottom-5"
            onClick={() => handleOperate(OperateType.DOWNLOAD, record, TTestVideoType.DYNAMIC_HOT)}
          >
            下载动态热力图
          </Button>
          {userInfo.role === 1 && (
            <Button
              ...
              className="margin-right-10 margin-bottom-5"
              danger
              onClick={() => handleOperate(OperateType.DELETE, record)}
            >
              删除
            </Button>
          )}
  ...
  const handleOperate = async (
    ...
    // 热力图查看详情、下载 + 下载动态热力图
    if (
      [OperateType.DETAIL, OperateType.DOWNLOAD].includes(type) &&
      [TTestVideoType.HOT, TTestVideoType.DYNAMIC_HOT].includes(currentType!)
    ) {
  ...
  const hotPictureOperate = async (
    ...
    const array = (
      currentType === TTestVideoType.HOT ? value.heatmapLink : value.heatmapVideoLink
    ).split("?");
    const ext = getExt(array.length > 1 ? array[0] : array);
    const filename = `${
      currentType === TTestVideoType.HOT ? "" : "动态"
    }热力图_(问卷ID_${surveyId})-(问题ID_${questionId})-(用户ID_${userId})-(视频ID_${id}).${
      ext || "jpg"
    }`;
    downloadFile(
      currentType === TTestVideoType.HOT ? value.heatmapLink : value.heatmapVideoLink,
   ...
   * @param {Record} data 单项数据体
   */
  const movePictureOperate = async (
    operateType: OperateType,
    currentType: TTestVideoType,
    data?: Record<string, any>
  ) => {
    ...
    if (operateType === OperateType.DOWNLOAD && !!data) {
      ...
      setTimeout(() => {
        ...
        chart.downloadImage(
          `轨迹图_(问卷ID_${surveyId})-(问题ID_${questionId})-(用户ID_${userId})-(视频ID_${id})`
        );
  ...
  const getMovePointData = async (data: Record<string, any>) => {
    ...
    const { rowPixel = 1080, columnPixel = 1670, statusBarHeight = 0 } = _result;
    return {
  ...
  const handleExportData = async (
    ...
    const [error, array] = await to(
      currentType === TTestVideoType.HOT ? downloadHotData(params) : donwloadMoveMapData(params)
    );
    if (error) {
      message.destroy();
    ...
    if (Array.isArray(array)) {
      ...
      array.forEach(({ videoId, indexList, surveyId, userId, questionId }) => {
        ...
        zip.file(
          `${
            currentType === TTestVideoType.HOT ? "热力图" : "轨迹图"
          }_(问卷ID_${surveyId})-(问题ID_${questionId})-(用户ID_${userId})-(视频ID_${videoId}).txt`,
          newIndexList.join("\n")
        );
      });
      zip.generateAsync({ type: "blob" }).then(function (content) {
        fileSaver.saveAs(
          content,
          `导出${currentType === TTestVideoType.HOT ? "热力图" : "轨迹图"}数据.zip`
        );
        message.destroy();
      ...
      <CustomSearch
        columns={[
          ...
          },
          {
            ...
            placeholder: "请输入时间段",
            customNode: (
              <DatePicker.RangePicker className="margin-top-5" style={{ width: "100%" }} />
            ),
          },
        ]}
    ...
    <div className="test-video-box">
      ...
      <Row className="mb-1 flex-wrap" justify={"end"}>
        <Button
          type="primary"
          className="mr-3 mb-3"
          onClick={() => moreDownloadOptions(TTestVideoType.HOT)}
        >
          批量下载热力图数据
        </Button>
        <Button
          type="primary"
          className="mr-3 mb-3"
          onClick={() => moreDownloadOptions(TTestVideoType.TRAJECTORY)}
        >
          批量下载轨迹图数据
        </Button>
        <Button
          ...
          onClick={() => exportChooseData(TTestVideoType.HOT)}
        >
          导出选中热力图数据
        </Button>
        <Button
          className="mr-3 mb-3"
          disabled={selectedArray.length === 0}
          onClick={() => exportChooseData(TTestVideoType.TRAJECTORY)}
        >
          导出选中轨迹图数据
        </Button>
      </Row>
